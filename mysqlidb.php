...
class MysqliDb
{
	...
	$rv = ($stmt->affected_rows > -1);

        $stmt->close();
        return $rv;	//	affected_rows returns 0 if nothing matched where statement, or required updating, -1 if error
    }
	...
	//print "Final Query: ".$this->_query;
	//print_r($this->_bindParams);

	//TBD, this should be done only if lastQuery was requested, else wastomh resources
